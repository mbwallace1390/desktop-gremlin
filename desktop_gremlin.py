...
class App:
    ...
    def physics(self, f, dt, gy):
        ...
        hit_wall = False
        if f.x < self.ox + mrg:
            f.x, f.vx, hit_wall = self.ox + mrg, abs(f.vx) * .4, True
        if f.x > self.ox + self.W - mrg:
            f.x, f.vx, hit_wall = self.ox + self.W - mrg, -abs(f.vx) * .4, True
        if f.y < self.oy + 18:
            f.y, f.vy = self.oy + 18, abs(f.vy) * .3
